...
module.exports = {
  getDataAll: (limit, offset, search, sort) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT *, COUNT(*) AS jumlah_skill FROM workers JOIN skills ON workers.worker_id = skills.worker_id WHERE skills.skill_name LIKE '%${search}%' ORDER BY ${sort} LIMIT ${limit} OFFSET ${offset} `,
        (error, result) => {
